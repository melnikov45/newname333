...
$query = "
    SELECT 
        p.mid AS person_id, -- Добавляем ID человека
        ...
        pr.name AS period_name -- Получаем название периода
    ...
    JOIN periods AS pr ON nnz_s.period = pr.lid -- Присоединяем таблицу периодов
...
$order = ["2-3 пара", "3-4 пара", "4-5 пара", "1-2 пара"];
$subjectsByHour = [];
foreach ($order as $period) {
    foreach ($data as $row) {
        if ($row['period_name'] === $period) {
            $hourIndex = array_search($period, $order);
            $subjectsByHour[$hourIndex][] = [
                'person_id' => $row['person_id'], // Добавляем ID человека
                'course_alias' => $row['course_alias'],
                'room_name' => $row['room_name'],
                'event_type_alias' => $row['event_type_alias'],
...
// Преобразуем в формат, который будет использоваться в JavaScript
$subjects = [];
for ($i = 0; $i < count($order); $i++) {
    $subjects[] = isset($subjectsByHour[$i]) ? $subjectsByHour[$i] : []; // Заполняем пустыми значениями, если нет предметов
}
...
<div id="myModal2" class="modal2">
    ...
    document.getElementById('link-button').addEventListener('click', function() {
        ...
        const selectedUsers = Array.from(document.querySelectorAll('.my-checkbox_2:checked')).map(checkbox => {
            return {
                id: checkbox.id.split('_')[2], // Извлекаем ID из id чекбокса
                fullName: checkbox.value
            };
        });

        // Пример данных subjects, полученных из PHP
        const subjects = <?php echo $subjectsJson; ?>; // Получаем данные из PHP

        // Генерация строк таблицы для каждого выбранного пользователя
        selectedUsers.forEach(user => {
            const userSubjects = subjects.map(hourSubjects => {
                return hourSubjects.filter(subject => subject.person_id == user.id); // Фильтруем по ID
            });

            userSubjects.forEach((hourSubjects, index) => {
                const row = document.createElement('tr');
                if (index === 0) {
                    const nameCell = document.createElement('td');
                    nameCell.rowSpan = userSubjects.length; // Объединяем ячейку для имени пользователя 
                    nameCell.textContent = user.fullName; // Заполняем ячейку полным именем
                    row.appendChild(nameCell);
                }
                const hourCell = document.createElement('td');
                hourCell.textContent = ["1-2 пара", "2-3 пара", "3-4 пара", "4-5 пара"][index]; // Обновляем часы
                row.appendChild(hourCell);

                // Добавляем предметы в ячейки
                hourSubjects.forEach(subjectData => {
                    const subjectCell = document.createElement('td');
                    subjectCell.textContent = subjectData.course_alias || ""; // Если предмета нет, оставляем пустую ячейку

                    // Определяем, является ли день выходным
                    const dayOfMonth = index + 1; // День месяца
                    ...
                    // Добавляем дополнительные данные в ячейку
                    subjectCell.innerHTML += `<br>${subjectData.room_name || ""}`;
                    subjectCell.innerHTML += `<br>${subjectData.event_type_alias || ""}`;
                    subjectCell.innerHTML += `<br>${subjectData.group_name || ""}`;
        ...
        function updateDayHeaders(daysInMonth) {
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                th.textContent = day; // Устанавливаем текст заголовка
                dayHeadersRow.appendChild(th); // Добавляем заголовок в строку
            }
        }
