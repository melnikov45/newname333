...
<body>
    <table id="data-table">
        <thead>
            <tr>
                <th>Фамилия, Имя, Отчество</th> <!-- Первый столбец -->
                <th>Данные</th> <!-- Второй столбец -->
            </tr>
        </thead>
        <tbody id="table-body">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            scheduleData.forEach(item => {
                ...
                // Создаем ячейку для Фамилии, Имени и Отчества
                const fullNameCell = document.createElement('td');
                fullNameCell.textContent = `${item.person_lastname} ${item.person_firstname} ${item.person_patronymic}`;
                row.appendChild(fullNameCell);

                // Создаем ячейку для остальных данных без названий
                const dataCell = document.createElement('td');
                dataCell.innerHTML = `
                    ${item.course_alias}<br>
                    ${item.room_name}<br>
                    ${item.event_type_alias}<br>
                    ${item.group_name}
                `;
